...
def render_rays(f_vert,
    ...
    np.savetxt('alpha_v_relu_np.txt', alpha_v.cpu().detach().numpy())
    alpha_g = FLAME_based_alpha_calculator_v_gauss(distances_v, m, eps)
    np.savetxt('alpha_v_gauss_np.txt', alpha_g.cpu().detach().numpy())
    alpha_s = FLAME_based_alpha_calculator_v_solid(distances_v, m, eps)
    np.savetxt('alpha_v_solid_np.txt', alpha_s.cpu().detach().numpy())
    # distances_f, idx_f = FLAME_based_alpha_calculator_3_face_version(pts, f_vert, f_faces, m, eps)
    # alpha_f = FLAME_based_alpha_calculator_2_face_version(distances_f, m, eps)
    # print('distances_v_s ',distances_v.size())
    # print('distances_f_s ', distances_f.size())
    # print('alpha_f_S ', alpha_f.size())
...
def train():
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            ...
            rgbs, _ = render_path(vertice, faces, render_poses, hwf, K, args.chunk, render_kwargs_test, gt_imgs=images,
    ...
    for i in trange(start, N_iters):
        ...
        if i % args.i_video == 0 and i > 0:
            ...
            with torch.no_grad():
                rgbs, disps = render_path(vertice, faces, render_poses, hwf, K, args.chunk, render_kwargs_test)
            ...
            #     with torch.no_grad():
            #         rgbs_still, _ = render_path(vertice, faces,, render_poses, hwf, args.chunk, render_kwargs_test)
        ...
        if i % args.i_testset == 0 and i > 0:
            ...
            # with torch.no_grad():
            render_path(vertice, faces, torch.Tensor(poses[i_test]).to(device), hwf, K, args.chunk, render_kwargs_test,
